...
sample_d2 = torch.zeros((sample_num,c_d2_num))

#第二个离散变量
sample_z2 = torch.rand((1, z_dim_num)).expand(sample_num, z_dim_num) #每个样本的z相同
temp = torch.zeros((c_d2_num, 1))
temp_d = torch.zeros((sample_num, 1))
sample_d1_2 = sample_d1

for i in range(c_d2_num):
	temp[i, 0] = i #每一个标签
...
if gpu_mode == True:
	...
	sample_z.cuda(), sample_d1.cuda(), sample_d2.cuda(), \
	sample_z2.cuda(), sample_d1_2.cuda(), sample_d2_2.cuda()

#------------------------model setting-----------------

G = model.generator_mwm3(z_dim=z_dim_num, output_channel=img_channel, input_size=input_size, len_discrete_code=c_d_num, len_continuous_code=c_d2_num)  
D = model.discriminator_mwm3(input_channel=img_channel, output_dim=1, input_size=input_size, len_discrete_code=c_d_num, len_continuous_code=c_d2_num)
...
# save2img
	with torch.no_grad():
		...
		samples = (samples + 1) / 2
		torchvision.utils.save_image(samples, save_dir + '/%d_Epoch-c_d2.png' % i, nrow=20)
